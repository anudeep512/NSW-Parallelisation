...
// mutex locks[100000];
// int entry_indexes[100000];
// map<int,int> entry_indexes[100000];
mutex entry_indexes_mtx;
...
void eachEntry(int threadID, int query_value, int K, int rem, int entries, vector<int> arr, int no_of_threads)
{

  for (int var = rem; var < entries; var = var + no_of_threads)
  {
...
set<int> KNN_Search(int query_value, int entries, int K)
{
  ...
  {
    threads[i] = thread(eachEntry, i + 1, query_value, K, i, entries, arr, no_of_threads);
  }
